...
| Fork bombs, cryptominers, infinite builds | Per-step and per-job timeouts with process-tree kill; PID/CPU/memory limits in Docker, rlimits on POSIX |
...
- **restricted** — used automatically when Docker is unavailable. It is a real
  ...
  container: code runs as the current OS user, and network access is not
  restricted. The interface states this before every run.
